...
describe('ngb-carousel', () => {
	...
	it('should render slides and navigation indicators', async () => {
		const html = `
      <ngb-carousel>
	...
	it('should mark the first slide as active by default', async () => {
		const html = `
      <ngb-carousel>
        <ng-template ngbSlide>foo</ng-template>
        <ng-template ngbSlide>bar</ng-template>
	...
	it('should work without any slides', async () => {
		const fixture = await createTestComponent(`<ngb-carousel [interval]="1000"></ngb-carousel>`);
	...
	it('should mark the requested slide as active', async () => {
     ...
     `;
		...
		const fixture = await createTestComponent(html, false);
	...
	it('should auto-correct when slide index is undefined', async () => {
		const html = `
  ...
  `;
	...
	it('should mark component for check for API calls', async () => {
      ...
      <button id="next" (click)="c.next(); addNewSlide.set(true)">Next</button>
    `;
   ...
   `;
	...
	it('should change slide on time passage in OnPush component (default interval value)', async () => {
		const fixture = await createTestComponent('<test-cmp-on-push></test-cmp-on-push>');
...
class TestComponent {
	readonly addNewSlide = signal(false);
	readonly interval = signal<number | undefined>(undefined);
	readonly activeSlideId = signal<string | undefined>(undefined);
	readonly keyboard = signal(true);
	readonly pauseOnHover = signal(true);
	readonly showNavigationArrows = signal(true);
	readonly showNavigationIndicators = signal(true);
	readonly slides = signal(['a', 'b']);
	readonly carouselSlideCallBack = (event: NgbSlideEvent) => {};
	readonly carouselSingleSlideCallBack = (event: NgbSingleSlideEvent, id: string) => {};
}
