...
describe('ngb-datepicker-service', () => {
	...
	let focusMove: (NgbMonth, NgbPeriod?, number?) => void;
	...
	beforeEach(() => {
		...
		model = <any>undefined;
		selectDate = <any>null;
		focusMove = (focusDate: NgbMonth, period?: NgbPeriod, number?: number) =>
		...
		// subscribing
		subscriptions.push(
			service.model$.subscribe(mock.onNext),
			service.model$.subscribe((m) => (model = m)),
			service.dateSelect$.subscribe(mockSelect.onNext),
			service.dateSelect$.subscribe((d) => (selectDate = d)),
		);
	});
	...
	describe(`min/max dates`, () => {
		it(`should emit null and valid 'minDate' values`, () => {
			...
			// invalid -> ignore
			service.set({ minDate: new NgbMonth(-2, 0, <any>null) });
			expect(model.minDate).toBeNull();
		...
		it(`should emit null and valid 'maxDate' values`, () => {
			...
			// invalid -> ignore
			service.set({ maxDate: new NgbMonth(-2, 0, <any>null) });
			expect(model.maxDate).toBeNull();
	...
	describe(`focus handling`, () => {
		...
		it(`should ignore invalid 'focus()' values`, () => {
			...
			service.focus(new NgbMonth(-2, 0, <any>null));
	...
	describe(`view change handling`, () => {
		it(`should open current month if nothing is provided`, () => {
			...
			service.open(null);
			expect(model.months.length).toBe(1);
			expect(model.firstDate).toEqual(new NgbMonth(today.year, today.month, 1));
			expect(model.focusDate).toEqual(today);
		});
