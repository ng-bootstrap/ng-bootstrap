...
import { NgbMonthpickerConfig } from './datepicker-config';
...
import { NgbMonthpickerDayView } from './datepicker-day-view';
import { NgbMonthpickerKeyboardService } from './datepicker-keyboard-service';
import { NgbMonthpickerNavigationSelect } from './datepicker-navigation-select';
import { NgbMonthpickerNavigation } from './datepicker-navigation';
...
function expectSelectedDate(element: DebugElement, selectedDate: NgbMonth | null) {
	...
	if (selectedDate) {
		...
		const day = days[0];
		const dayView = day.parent!.query(By.directive(NgbMonthpickerDayView)).componentInstance as NgbMonthpickerDayView;
...
function expectFocusedDate(element: DebugElement, focusableDate: NgbMonth, isFocused = true) {
	...
	// checking it corresponds to our date
	const dayView = day.query(By.directive(NgbMonthpickerDayView)).componentInstance as NgbMonthpickerDayView;
...
function expectSameValues(datepicker: NgbMonthpicker, config: NgbMonthpickerConfig) {
...
describe('ngb-monthpicker', () => {
	it('should initialize inputs with provided config', () => {
		...
		const datepicker = TestBed.createComponent(NgbMonthpicker).componentInstance;
	...
	it('should handle incorrect startDate values', () => {
		...
		fixture.componentInstance.date = new NgbMonth(3000000, 1, 1);
		fixture.detectChanges();
		expect(getMonthSelect(fixture.nativeElement).value).toBe(currentMonth);
		expect(getYearSelect(fixture.nativeElement).value).toBe(currentYear);
	});
	...
	it('should handle minDate edge case values', () => {
		const fixture = createTestComponent(`<ngb-datepicker [minDate]="minDate" [startDate]="date"></ngb-datepicker>`);
		const datepicker = fixture.debugElement.query(By.directive(NgbMonthpicker)).injector.get(NgbMonthpicker);
		...
		// resetting
		fixture.componentInstance.minDate = new NgbMonth(3000000, 1, 1);
		fixture.detectChanges();
		expectMinDate(1000, 1);
	...
	it('should handle maxDate edge case values', () => {
		...
		// resetting
		fixture.componentInstance.maxDate = new NgbMonth(3000000, 1, 1);
		fixture.detectChanges();
		expectMaxDate(10000, 1);
	...
	it('should display multiple months', () => {
		...
		let months = fixture.debugElement.queryAll(By.directive(NgbMonthpickerMonth));
		expect(months.length).toBe(1);

		fixture.componentInstance.displayMonths = 3;
		fixture.detectChanges();
		months = fixture.debugElement.queryAll(By.directive(NgbMonthpickerMonth));
	...
	it('should not focus day initially', () => {
		...
		const datepicker = fixture.debugElement.query(By.directive(NgbMonthpicker));
		expectFocusedDate(datepicker, new NgbMonth(2016, 8, 1), false);
	});
	...
	it('should remove focus day on blur', () => {
		...
		focusDay();
		fixture.detectChanges();
		expectFocusedDate(datepicker, new NgbMonth(2016, 8, 1), true);
	...
	describe('keyboard navigation', () => {
		...
		it('should move focus with arrow keys', () => {
			...
			fixture.detectChanges();
			expectFocusedDate(datepicker, new NgbMonth(2016, 8, 8));
			expectSelectedDate(datepicker, null);

			triggerKeyDown(getMonthContainer(datepicker), 39 /* right arrow */);
			fixture.detectChanges();
			expectFocusedDate(datepicker, new NgbMonth(2016, 8, 9));
			expectSelectedDate(datepicker, null);

			triggerKeyDown(getMonthContainer(datepicker), 38 /* up arrow */);
			fixture.detectChanges();
			expectFocusedDate(datepicker, new NgbMonth(2016, 8, 2));
			expectSelectedDate(datepicker, null);

			triggerKeyDown(getMonthContainer(datepicker), 37 /* left arrow */);
			fixture.detectChanges();
			expectFocusedDate(datepicker, new NgbMonth(2016, 8, 1));
			expectSelectedDate(datepicker, null);
		});
		...
		it('should select focused date with enter or space', () => {
			...
			fixture.detectChanges();
			expectFocusedDate(datepicker, new NgbMonth(2016, 8, 1));
			expectSelectedDate(datepicker, new NgbMonth(2016, 8, 1));
			...
			fixture.detectChanges();
			expectFocusedDate(datepicker, new NgbMonth(2016, 8, 8));
			expectSelectedDate(datepicker, new NgbMonth(2016, 8, 8));
		});
		...
		it('should select first and last dates of the view with home/end', () => {
			...
			fixture.detectChanges();
			expectFocusedDate(datepicker, new NgbMonth(2016, 9, 30));
			expectSelectedDate(datepicker, null);
		...
		it('should select min and max dates with shift+home/end', () => {
			...
			fixture.detectChanges();
			expectFocusedDate(datepicker, new NgbMonth(2020, 12, 31));
			expectSelectedDate(datepicker, null);
			...
			fixture.detectChanges();
			expectFocusedDate(datepicker, new NgbMonth(2010, 1, 1));
			expectSelectedDate(datepicker, null);
		...
		it('should navigate between months with pageUp/Down', () => {
			...
			let datepicker = fixture.debugElement.query(By.directive(NgbMonthpicker));
			...
			fixture.detectChanges();
			expectFocusedDate(datepicker, new NgbMonth(2016, 7, 2));
			expectSelectedDate(datepicker, null);
			...
			expectFocusedDate(datepicker, new NgbMonth(2016, 9, 2));
			expectSelectedDate(datepicker, null);

			triggerKeyDown(getMonthContainer(datepicker), 34 /* page down */);
			fixture.detectChanges();
			datepicker = fixture.debugElement.query(By.directive(NgbMonthpicker));
			expectFocusedDate(datepicker, new NgbMonth(2016, 10, 2));
			expectSelectedDate(datepicker, null);
		});
		...
		it('should navigate between years with shift+pageUp/Down', () => {
			...
			expectFocusedDate(datepicker, new NgbMonth(2015, 8, 1), true);
			expectSelectedDate(datepicker, null);
	...
	describe('forms', () => {
      ...
      `);
		...
		it('should not change again the value in the model on a change coming from the model (template-driven form)', () => {
			...
			const value = new NgbMonth(2018, 7, 28);
		...
		it('should not change again the value in the model on a change coming from the model (reactive form)', () => {
			...
			const controlValue = new NgbMonth(2018, 7, 28);
	...
	describe('NgbMonthpicker', () => {
		...
		const mockKeyboardService: Partial<NgbMonthpickerKeyboardService> = {
			processKey(event: KeyboardEvent, datepicker: NgbMonthpicker) {
		...
		beforeEach(() => {
			...
			mv = fixture.debugElement.query(By.css('ngb-datepicker-month')).injector.get(NgbMonthpickerMonth);
		});

		it('should provide an defensive copy of minDate', () => {
			mv.onKeyDown(<KeyboardEvent>{});
			expect(mockState.firstDate).toEqual(NgbMonth.from({ year: 2016, month: 8, day: 1 })!);
			expect(mockState.lastDate).toEqual(NgbMonth.from({ year: 2016, month: 8, day: 31 })!);
			expect(mockState.minDate).toEqual(NgbMonth.from({ year: 2010, month: 1, day: 1 }));
			expect(mockState.maxDate).toEqual(NgbMonth.from({ year: 2020, month: 12, day: 31 }));
			Object.assign(mockState, { minDate: undefined });
			mv.onKeyDown(<KeyboardEvent>{});
			expect(dp.model.minDate).toEqual(NgbMonth.from({ year: 2010, month: 1, day: 1 }));
		});
		...
		it('should provide an defensive copy of maxDate', () => {
			...
			mv.onKeyDown(<KeyboardEvent>{});
			expect(dp.model.maxDate).toEqual(NgbMonth.from({ year: 2020, month: 12, day: 31 }));
		});
		...
		it('should provide an defensive copy of firstDate', () => {
			...
			mv.onKeyDown(<KeyboardEvent>{});
			expect(dp.model.firstDate).toEqual(NgbMonth.from({ year: 2016, month: 8, day: 1 }));
		});
		...
		it('should provide an defensive copy of lastDate', () => {
			...
			mv.onKeyDown(<KeyboardEvent>{});
			expect(dp.model.lastDate).toEqual(NgbMonth.from({ year: 2016, month: 8, day: 31 }));
		});

		it('should provide an defensive copy of focusedDate', () => {
			mv.onKeyDown(<KeyboardEvent>{});
			expect(mockState.focusedDate).toEqual(NgbMonth.from({ year: 2016, month: 8, day: 1 })!);
			Object.assign(mockState, { focusedDate: undefined });
			mv.onKeyDown(<KeyboardEvent>{});
			expect(dp.model.focusDate).toEqual(NgbMonth.from({ year: 2016, month: 8, day: 1 }));
		});
...
class TestComponent {
	...
	displayMonths = 1;
	navigation = 'select';
	minDate: NgbMonthStruct = { year: 2010, month: 1, day: 1 };
	maxDate: NgbMonthStruct = { year: 2020, month: 12, day: 31 };
	...
	markDisabled = (date: NgbMonthStruct) => {
		return NgbMonth.from(date)!.equals(new NgbMonth(2016, 8, 22));
	};
	onNavigate = (event) => {};
	onDateSelect = () => {};
