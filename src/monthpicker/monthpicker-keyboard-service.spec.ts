...
describe('ngb-monthpicker-keyboard-service', () => {
	...
	let state: NgbMonthpickerState = Object.assign({ focusedDate: { day: 1, month: 1, year: 2018 } });
	...
	it('should move focus to the first and last days in the view with "Home" and "End"', () => {
		...
		Object.assign(state, { firstDate: new NgbMonth(2017, 1) });
		Object.assign(state, { lastDate: new NgbMonth(2017, 12) });

		processKey(event(Key.Home));
		expect(mock.focusDate).toHaveBeenCalledWith(new NgbMonth(2017, 1));

		processKey(event(Key.End));
		expect(mock.focusDate).toHaveBeenCalledWith(new NgbMonth(2017, 12));
	...
	it('should move focus to the "min" and "max" dates with "Home" and "End"', () => {
		...
		Object.assign(state, { minDate: new NgbMonth(2017, 1) });
		Object.assign(state, { maxDate: new NgbMonth(2017, 12) });
