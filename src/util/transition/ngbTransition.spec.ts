...
if (isBrowserVisible('ngbRunTransition')) {
  describe('ngbRunTransition', () => {
    ...
    it(`should execute callbacks in provided zone with 'animations: false'`, () => {
      ...
      const next = createSpy('next', (_: any) => expect(NgZone.isInAngularZone()).toBe(true)).and.callThrough();
      const error = createSpy();
      const complete = createSpy('complete', () => expect(NgZone.isInAngularZone()).toBe(true)).and.callThrough();
    ...
    it(`should execute callbacks in provided zone when stopping currently running transition'`, () => {
      ...
      const completeSpy1 = createSpy('complete', () => expect(NgZone.isInAngularZone()).toBe(true)).and.callThrough();
