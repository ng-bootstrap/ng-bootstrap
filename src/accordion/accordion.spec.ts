...
function getPanelsContent(element: HTMLElement): HTMLDivElement[] {
	return <HTMLDivElement[]>(
		Array.from(element.querySelectorAll('.accordion-item > .collapse, .accordion-item > .collapsing'))
	);
}
...
describe('ngb-accordion', () => {
  ...
  `;
    ...
    `;
		...
		const titles = getPanelsTitle(fixture.nativeElement);
		titles.forEach((title, idx) => {
			expect(title).toBe(`Header Title ${idx + 1}`);
		});
	});
	...
	it('should not crash for panels without content', () => {
		const fixture = createTestComponent(
			'<ngb-accordion activeIds="open_me"><ngb-panel id="open_me"></ngb-panel></ngb-accordion>',
		);
	...
	it('should emit panel events when toggling panels', () => {
		...
		getButton(fixture.nativeElement, 0).click();
		fixture.detectChanges();
		expect(fixture.componentInstance.changeCallback).toHaveBeenCalledWith(
			jasmine.objectContaining({ panelId: 'one', nextState: true }),
		);
		...
		fixture.detectChanges();
		expect(fixture.componentInstance.changeCallback).toHaveBeenCalledWith(
			jasmine.objectContaining({ panelId: 'one', nextState: false }),
		);
...
@Component({ selector: 'test-cmp', standalone: true, imports: [NgbAccordionModule, NgForOf], template: '' })
class TestComponent {
	...
	preventDefaultCallback = (event: NgbPanelChangeEvent) => {
		event.preventDefault();
	};
}
